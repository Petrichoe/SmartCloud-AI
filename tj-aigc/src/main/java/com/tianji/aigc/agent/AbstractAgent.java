...
@Slf4j
public abstract class AbstractAgent implements Agent { //使用抽象类可以避免被实例化
        ...
        return this.getChatClientRequest(sessionId, requestId, question)
                ...
                .map(chatResponse -> {
                    ...
                    if (StrUtil.equals(Constant.STOP, finishReason)) {
                        // 通过反射获取 metadata 字段，添加 null 检查
                        Object metadataObj = ReflectUtil.getFieldValue(chatResponse, Constant.Chats.CHAT_RESPONSE_METADATA);
                        if (metadataObj instanceof ChatResponseMetadata metadata) {
                            var messageId = metadata.getId();
                            if (StrUtil.isNotBlank(messageId)) {
                                ToolResultHolder.put(messageId, Constant.REQUEST_ID, requestId);
                            }
                        } else {
                            log.warn("ChatResponseMetadata is null or invalid, unable to store requestId mapping");
                        }
                    }
                    // 获取大模型的输出的内容
